<div class="modal fade bs-example-modal-md updateApplicationStatusModal" tabindex="-1" role="dialog"
     aria-labelledby="mySmallModalLabel" aria-hidden="true" style="display: none;">
    <div class="modal-dialog modal-md">
        <div class="modal-content">
            <div class="modal-header">
                <h4>{{__('تحديث حالة الطلب')}}</h4>
            </div>
            <form class="updateAppForm" method="post"><input class="updateAppId" type="hidden" name="id">
                @csrf
                <div class="modal-body">
                    <div class="modal-body">
                        <div class="col-md-12">
                            <label class="required">{{__("أختيار الحالة")}}</label>
                            <div class="">
                                <select id="applicationStatusSelect" style="height: 40px !important;" required
                                        class="form-control" name="status">
                                    <option value="">حدد حالة الطلب</option>
                                    <option value="2"> الطلب مقبول</option>
                                    <option value="3"> الطلب غير كامل البيانات</option>
                                    <option value="4"> الطلب غير مكتمل الشروط</option>
                                </select>
                            </div>
                        </div>


{{--                        <div style="display: none" id="applicationStatusAssignEmployeeTypeSelectDiv"--}}
{{--                             class="col-md-12 pt-3">--}}
{{--                            <label class="required">{{__("ارسال الطلب الي")}}</label>--}}
{{--                            <div class="">--}}
{{--                                <select id="applicationStatusAssignEmployeeTypeSelect" style="height: 40px !important;"--}}
{{--                                        class="form-control" name="assign_employee_type">--}}
{{--                                    <option value="social"> مدير الإيجار</option>--}}
{{--                                    <option value="manager">المدير التنفيذي</option>--}}
{{--                                    <option value="money">مدير المالية</option>--}}
{{--                                </select>--}}
{{--                            </div>--}}
{{--                        </div>--}}

                        <div class="col-md-12 pt-3">
                            @includeIf('admin.components.form.add.textarea', [
                              'icon' => 'fa fa-user',
                              'label' => trans('راي الباحث'),
                              'name' => 'notes',
                              'placeholder'=>trans('راي الباحث')
                          ])
                        </div>
                    </div>
                </div>
                <div class="modal-footer">
                    <button type="submit" class="btn btn-success waves-effect waves-light">{{__('تحديث')}} </button>
                    <button type="button" class="btn btn-default waves-effect"
                            data-dismiss="modal">{{__('language.close')}}</button>
                </div>
            </form>
        </div>
    </div>
</div>
